...
def subtile_renamer(file_path, subtitle_language_code, istest=False):
    ...
    def find_episode(file: str) -> str:
        # find episode number in file name

        episode = re.findall(r"[S|s]([0-9][0-9])[E|e]([0-9][0-9])", file)
        # match pattern S(Season)E(Episode) and return the episode number
        ...
        # match pattern [(Season)x(Episode)] and return the episode number
        if episode.__len__() != 0:
            season_num = episode[0][0]
            episode_num = episode[0][1]
            return f"S{season_num}E{episode_num}"
    ...
    for file in files:
        ...
        t_file_extention = re.findall(
            r"\.([a-zA-z0-9]{3,3})", file_extention)[0].lower()
    ...
    # get sutitle file new name from video file
...
if __name__ == "__main__":
    ...
    paser.add_argument('-test', '-t', action='store_true',
                       help='test before rename')
    args = paser.parse_args()

    file_path = args.filePath
    subtitle_language_code = args.language
    istest = args.test
    subtile_renamer(file_path=file_path,
                    subtitle_language_code=subtitle_language_code, istest=istest)
